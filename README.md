...
**🗄️ 存储架构升级**
- MySQL 改为主存储，JSON 文件降级为备用方案
- 数据优先读写 MySQL，仅在数据库不可用时自动回退 JSON，保障服务连续性

**🛡️ 管理员面板**
- 新增管理员权限体系：被赋予管理员权限的用户可查看全平台数据
- 涵盖用户统计、对话总量、学习行为等全维度运营数据

**👤 用户体系完善**
- **头像修复**：修复更换头像后刷新页面丢失的 bug，新增头像文件格式与尺寸校验，不合规时给出人性化提示
- **用户名唯一性**：注册与修改用户名时校验全局唯一，防止重复
- **密码安全升级**：密码最低要求提升至 8 位，新增密码强度实时提醒（弱 / 中 / 强）

**🔗 数据库连接健壮性**
- 修复 MySQL 长时间无活动被服务器断开连接的问题：引入连接池心跳保活与自动重连机制
- 优化连接生命周期管理，防止僵尸连接堆积占用资源

**🐛 Bug 修复**
- **管理员面板**：修复总对话数未去重导致数据虚高；修复前端创建欢迎对话逻辑错误导致的统计偏差
- **HTTP Header**：处理 Header 中包含中文字符导致请求异常的问题，自动进行编码兼容
