...
export function ExtendedModule() {
  ...
  const projectCache = useRef<Record<string, any>>({});
  ...
  const closeProject = () => { setProjectModal(null); setProjectDetail(null); };

  // 点击项目卡片：打开弹窗并触发 AI 生成详细内容
  const openProject = async (project: typeof projectTemplates[0][0]) => {
    setProjectModal(project);
    setProjectDetail(null);
    const cacheKey = `${project.id}-${project.title}`;
    // 先检查缓存
    if (projectCache.current[cacheKey]) {
      setProjectDetail(projectCache.current[cacheKey]);
      return;
    }
    setIsGeneratingProject(true);
    try {
      const topic = `${topicName} - ${project.title}`;
      const res = await fetch("/api/generate-resource", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...authHeaders },
        body: JSON.stringify({ userProfile, topic, type: "project" }),
      });
      const result = await res.json();
      if (!res.ok) throw new Error(result.error || "生成失败");
      if (result.data) {
        projectCache.current[cacheKey] = result.data;
        setProjectDetail(result.data);
      }
    } catch (e: any) {
      console.error("[Project] Generate failed:", e);
      setToastStr("项目详情生成失败：" + (e.message || "请重试"));
      setTimeout(() => setToastStr(""), 3000);
    } finally {
          ...
          <div className="flex items-start justify-center min-h-full py-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl border border-slate-100 animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
              ...
              <div className="px-6 sm:px-8 py-5 space-y-5">
                ...
                {isGeneratingProject && (
                  <div className="flex flex-col items-center justify-center py-16 gap-5 animate-in fade-in">
                    ...
                    <div className="text-center">
                      <p className="font-bold text-slate-700 text-sm mb-1">AI 正在生成项目详情</p>
                      <p className="text-xs text-slate-400">分析知识点 → 设计技术方案 → 规划实现步骤</p>
                    </div>
                ...
                {/* AI 生成的项目详情 */}
                {!isGeneratingProject && projectDetail && (
                  <>
                    {/* 项目描述 */}
                    <div className="p-5 bg-gradient-to-br from-slate-50 to-white rounded-2xl border border-slate-200/60">
                      ...
                      <div className="text-[14px] text-slate-600 leading-relaxed prose prose-sm max-w-none prose-li:text-slate-600 prose-strong:text-slate-800">
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>{projectDetail.desc}</ReactMarkdown>
                      </div>
                    </div>
                    ...
                    <div className="p-5 bg-gradient-to-br from-emerald-50/80 to-teal-50/80 rounded-2xl border border-emerald-100/50 shadow-sm">
                      ...
                      <div className="text-[14px] text-slate-600 leading-relaxed prose prose-sm max-w-none prose-li:text-slate-600 prose-strong:text-slate-800">
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>{projectDetail.goals}</ReactMarkdown>
                      </div>
                    </div>
                    ...
                    <div className="p-5 bg-white border border-slate-200/50 rounded-2xl shadow-sm">
                      ...
                      <div className="text-[14px] text-slate-600 leading-relaxed prose prose-sm max-w-none prose-ol:space-y-2 prose-li:text-slate-700 prose-strong:text-slate-800">
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>{projectDetail.steps}</ReactMarkdown>
                      </div>
                    </div>
                    ...
                    <div className="p-5 bg-amber-50/40 rounded-2xl border border-amber-100/50">
                      ...
                      <div className="text-[14px] text-slate-600 leading-relaxed prose prose-sm max-w-none prose-li:text-slate-700">
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>{projectDetail.criteria}</ReactMarkdown>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-blue-50/40 rounded-2xl border border-blue-100/50 flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[14px] text-slate-600 leading-relaxed prose prose-sm max-w-none">
                          <ReactMarkdown remarkPlugins={[remarkGfm]}>{projectDetail.tips}</ReactMarkdown>
                        </div>
                      </div>
                    </div>
                  </>
                  ...
                  <>
                    ...
                    <div className="p-5 bg-white border border-slate-200/50 rounded-2xl shadow-sm">
                      ...
                      <div className="space-y-3">
                        {getProjectSteps(projectModal.difficulty).map((step: string, idx: number) => (
