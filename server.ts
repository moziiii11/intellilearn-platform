...

{
  ...
  "knowledgeBaseText": "<2-3句分析>",
  "errorProneAreasText": "<2-3句薄弱点分析和建议>",
  "learningGoalsText": "<2-3句目标分析和建议>",
...
async function startServer() {
  ...
  app.post("/api/auth/register", async (req, res) => {
    const { username, password, birthday, primarySchool } = req.body;
    if (!username || !password) {
      return res.status(400).json({ success: false, message: "用户名和密码不能为空" });
    }
    ...
    // 优先使用 MySQL 存储
    if (mysqlAvailable) {
      const created = await mysqlCreateUser(username, password, birthday || "", primarySchool || "");
      if (!created) {
        return res.status(400).json({ success: false, message: "用户名已存在" });
      }
      // 同步到 JSON DB，确保其他功能正常
      const db = readDB();
      db.users[username] = { password, birthday: birthday || "", primarySchool: primarySchool || "" };
      writeDB(db);
    ...
    if (db.users[username]) {
      ...
      db.users[username] = { password, birthday, primarySchool };
      writeDB(db);
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    const { username, birthday, primarySchool, newPassword } = req.body;
    ...
    // 优先使用 MySQL
    if (mysqlAvailable) {
      const verified = await mysqlVerifyUser(username, birthday || "", primarySchool || "");
      if (!verified) {
        return res.status(400).json({ success: false, message: "验证信息错误" });
      }
      await mysqlUpdatePassword(username, newPassword);
      // 同步到 JSON DB
      const db = readDB();
      if (db.users[username]) {
    ...
    // 回退到 JSON DB
    const db = readDB();
    if (db.users[username] && db.users[username].birthday === birthday && db.users[username].primarySchool === primarySchool) {
      db.users[username].password = newPassword;
      writeDB(db);
      res.json({ success: true, message: "密码重置成功" });
    } else {
      res.status(400).json({ success: false, message: "验证信息错误" });
    }
  });
